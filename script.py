...
#run = wandb.init(project="whisper_finetuning", job_type="fine-tuning", group="medium-indo")
@dataclass
class Lang:
    name: str
    lang_id: str
        
indonesia_cv = Lang(name="indonesian", lang_id="id")
indonesia_magic = Lang(name="indonesian", lang_id="id")
indonesia_magic_conversation = Lang(name="indonesian", lang_id="id")
indonesia_titml =  Lang(name="indonesian", lang_id="id")
indonesia_fluers =  Lang(name="indonesian", lang_id="id")
english = Lang(name="english", lang_id="en")
languages = [indonesia_cv,indonesia_magic,indonesia_magic_conversation,indonesia_titml, english]
...
def load_english_dataset(is_streaming=True, stopping_strategy="all_exhausted"):
    dataset_dict = {}    
    dataset_dict["train"] = load_dataset("google/fleurs", "en_us", split="train",ignore_verifications=True, streaming=is_streaming)
    dataset_dict["train"] = dataset_dict["train"].rename_column("raw_transcription", "sentence")
    dataset_dict["train"] = dataset_dict["train"].remove_columns(
        [col for col in dataset_dict["train"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["train"] = dataset_dict["train"]

    dataset_dict["test"] = load_dataset("google/fleurs", "en_us",ignore_verifications=True, split="test", streaming=is_streaming)
    dataset_dict["test"] = dataset_dict["test"].rename_column("raw_transcription", "sentence")
    ...
    dataset_dict = preprocess_dataset(dataset_dict, lang="en")
    return dataset_dict

def load_indonesia_fleurs(is_streaming=True, stopping_strategy="all_exhausted"):
    dataset_dict = {}    
    dataset_dict["train"] = load_dataset("google/fleurs", "id_id",ignore_verifications=True, split="train", streaming=is_streaming)
    dataset_dict["train"] = dataset_dict["train"].rename_column("raw_transcription", "sentence")
    dataset_dict["train"] = dataset_dict["train"].remove_columns(
        [col for col in dataset_dict["train"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["train"] = dataset_dict["train"]

    dataset_dict["test"] = load_dataset("google/fleurs", "id_id",ignore_verifications=True, split="test", streaming=is_streaming)
    dataset_dict["test"] = dataset_dict["test"].rename_column("raw_transcription", "sentence")
    dataset_dict["test"] = dataset_dict["test"].remove_columns(
        [col for col in dataset_dict["test"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["test"] = dataset_dict["test"]
    dataset_dict = preprocess_dataset(dataset_dict, lang="id")
    return dataset_dict

def load_indonesia_cv_dataset(is_streaming=True, stopping_strategy="all_exhausted"):
    dataset_dict = {}    
    dataset_dict["train"] = load_dataset("mozilla-foundation/common_voice_11_0","id",ignore_verifications=True, split="train", streaming=is_streaming)
    dataset_dict["train"] = dataset_dict["train"].remove_columns(
        [col for col in dataset_dict["train"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["train"] = dataset_dict["train"]

    dataset_dict["test"] = load_dataset("mozilla-foundation/common_voice_11_0", "id",ignore_verifications=True, split="test", streaming=is_streaming)
    dataset_dict["test"] = dataset_dict["test"].remove_columns(
        [col for col in dataset_dict["test"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["test"] = dataset_dict["test"]
    dataset_dict = preprocess_dataset(dataset_dict, lang="id")
...
def load_indonesia_magic_dataset(is_streaming=True, stopping_strategy="all_exhausted"):
    dataset_dict = {}    
    dataset_dict["train"] = load_dataset("/root/whisper_finetuning/local_dataset/magic_data/","id", split="train", streaming=is_streaming)
    dataset_dict["train"] = dataset_dict["train"].remove_columns(
        [col for col in dataset_dict["train"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["train"] = dataset_dict["train"]
...
def load_indonesia_magic_conversation_dataset(is_streaming=True,stopping_strategy="all_exhausted"):
    dataset_dict = {}    
    dataset_dict["train"] = load_dataset("/root/whisper_finetuning/local_dataset/magic_data_conversation/","id", split="train", streaming=is_streaming)
    dataset_dict["train"] = dataset_dict["train"].remove_columns(
        [col for col in dataset_dict["train"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["train"] = dataset_dict["train"]
...
def load_indonesia_titml(is_streaming=True, stopping_strategy="all_exhausted"):
    dataset_dict = {}    
    dataset_dict["train"] = load_dataset("/root/whisper_finetuning/local_dataset/titlm","id", split="train", streaming=is_streaming)
    dataset_dict["train"] = dataset_dict["train"].remove_columns(
        [col for col in dataset_dict["train"].features.keys() if col not in ["audio", "sentence",]])
    dataset_dict["train"] = dataset_dict["train"]
...
def load_dataset_splits(is_streaming=True, stopping_strategy="all_exhausted"):
    english_dataset = load_english_dataset(is_streaming, stopping_strategy)
    indonesian_fleurs = load_indonesia_fleurs(is_streaming, stopping_strategy)
    cv_id_dataset = load_indonesia_cv_dataset(is_streaming, stopping_strategy)
    magic_data_dataset = load_indonesia_magic_dataset(is_streaming, stopping_strategy)
    magic_data_conversation_dataset = load_indonesia_magic_conversation_dataset(is_streaming, stopping_strategy)
    titml_dataset = load_indonesia_titml(is_streaming, stopping_strategy)
    dataset_dict = IterableDatasetDict()
    for split in ["train"]:
        dataset_dict[split] = interleave_datasets(
            [
                titml_dataset[split],
                magic_data_dataset[split],
                indonesian_fleurs[split],
                english_dataset[split],
                cv_id_dataset[split],
                magic_data_conversation_dataset[split]
...
model = WhisperForConditionalGeneration.from_pretrained(MODEL_NAME, use_cache=False)
...
training_args = Seq2SeqTrainingArguments(
    output_dir="models/whisper-medium-indo",  # change to a repo name of your choice
